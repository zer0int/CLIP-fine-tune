...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if self.head_mask is not None:
            # self.head_mask: [num_heads] of {0,1}, broadcast to [N * num_heads, 1, 1]
            mask_flat = self.head_mask.to(v.device).float().repeat(N).view(N * num_heads, 1, 1)
            v = v * mask_flat
        ...
        # register the provided hook function (and only if grads exist)
        ...
        if need_weights:
            attn_output_weights = attn_output_weights.view(N, num_heads, tgt_len, src_len)
            return attn_output, attn_output_weights  # <--- NO AVERAGING!
        else:
...
def build_model(state_dict: dict):
    ...
    if use_positional_embedding_res:
        ...
        # If not using Long-CLIP, drop unexpected keys so strict loading stays works
